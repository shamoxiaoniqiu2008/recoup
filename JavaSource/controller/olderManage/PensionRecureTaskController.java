...
import service.olderManage.PensionRecureTaskService;
 ...
 */
public class PensionRecureTaskController implements Serializable {

	private static final long serialVersionUID = 1L;

	private transient PensionRecureTaskService pensionRecureTaskService;
	...
	private List<PensionOlderRecureExtend> myRecureDetailList = new ArrayList<PensionOlderRecureExtend>();
	...
	private PensionOlderRecureExtend oneRecureDetail;
	/**
	 * 输入法sql
	 */
	...
	public void searchRecureRecords() {
		...
		//olderRecureMainExtendList = pensionRecureTaskService.selectOlderRecureMain(olderId, recureitemId, handle);
		myRecureDetailList = pensionRecureTaskService.selectMyRecureDetail(curUser.getId(), start, end, handle);
		//selectMyRecureDetail
	}
	/**
	 * 初始化输入法sql
	 */
	...
	/*public void saveRecureDetail() {
		if(olderRecureMainExtend.getOlderId() != null && 
			...
			//List<PensionRecureDetail> asfd = pensionRecureTaskService.selectRecureDetail(olderRecureMainExtend.getRecureitemId());
		
			try {
				pensionRecureTaskService.updatePensionOlderRecure(olderRecureMainExtend, olderRecureDetailList);
				olderRecureDetailList = pensionRecureTaskService.selectOlderRecureDetail(olderRecureMainExtend.getId());
				searchRecureRecords();
...
*/
	...
	public void saveRecureDetail() {
		try {
			oneRecureDetail.setHandlenurse(curUser.getId()); //实际处理人
			pensionRecureTaskService.updatePensionOlderRecure(oneRecureDetail);
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "保存成功", ""));
		} catch (PmsException e) {
			// TODO Auto-generated catch block
	...
	public void showRecureDetail() {
		olderRecureMainExtend = pensionRecureTaskService.selectRecureMain(oneRecureDetail.getRecureMainId());
		olderRecureDetailList = pensionRecureTaskService.selectOlderRecureDetail(oneRecureDetail.getRecureMainId());
	}
	...
	public PensionRecureTaskService getPensionRecureTaskService() {
		return pensionRecureTaskService;
	}

	public void setPensionRecureTaskService(
			PensionRecureTaskService pensionRecureTaskService) {
		this.pensionRecureTaskService = pensionRecureTaskService;
	}
	...
	public void setMyRecureDetailList(List<PensionOlderRecureExtend> myRecureDetailList) {
		this.myRecureDetailList = myRecureDetailList;
	}

	public List<PensionOlderRecureExtend> getMyRecureDetailList() {
		return myRecureDetailList;
	}
	...
	public void setOneRecureDetail(PensionOlderRecureExtend oneRecureDetail) {
		this.oneRecureDetail = oneRecureDetail;
	}

	public PensionOlderRecureExtend getOneRecureDetail() {
		return oneRecureDetail;
	}
