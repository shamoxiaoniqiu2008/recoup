 ...
 */
@Service
public class ViewRoomMapService {
	@Autowired
	 ...
	 */
	public List<PensionRoom> selectRooms(Long floorId) {
	...
	public List<PensionBuilding> selectBuildList() {
		...
		for (PensionBuilding build : buildList) {
			...
			for (PensionFloor floor : floorList) {
				...
				for (PensionBed bed : bedList) {
					List<PensionOlderDomain> olderList = new ArrayList<PensionOlderDomain>();
					olderList = selectOlderList(bed.getId());
					bed.setOlderList(olderList);
				}
			}
