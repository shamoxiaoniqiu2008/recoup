...
@Service
public class ViewVisitationRecordService {
	@Autowired
	 ...
	 * @return
	 */
	public List<PensionOlderDomain> selectOlderRecords(Date startDate,
			Date endDate, String visitCatalog, String visitFlag) {
		...
		map.put("visitCatalog", visitCatalog);
		map.put("visitFlag", visitFlag);
		return pensionOlderMapper.selectOlderRecords(map);
	}
	...
	public List<PensionVisitrecord> selectUnVisitRecords(Long keyId) {
		PensionVisitrecordExample example = new PensionVisitrecordExample();
		example.or().andKeyIdEqualTo(keyId).andClearedEqualTo(2)
				.andHandleFlagEqualTo(2);
		return pensionVisitrecordMapper.selectByExample(example);
	}
